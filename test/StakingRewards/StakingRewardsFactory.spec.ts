...
describe("StakingRewardsFactory", () => {
  ...
  it("deployment gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq("3654763");
  });
  ...
  describe("#deploy", () => {
    ...
    it("fails if called twice for same token", async () => {
      await stakingRewardsFactory.deploy(stakingTokens[1].address, rewardTokens[0].address, 10000, REWARDS_DURATION);
      await expect(
        stakingRewardsFactory.deploy(stakingTokens[1].address, rewardTokens[0].address, 10000, REWARDS_DURATION),
      ).to.revertedWith("StakingRewardsFactory::deploy: already deployed");
    });
    ...
    it("can only be called by the owner", async () => {
      await expect(
        stakingRewardsFactory
          .connect(wallet1)
          .deploy(stakingTokens[1].address, rewardTokens[0].address, 10000, REWARDS_DURATION),
  ...
  describe("#notifyRewardsAmounts", () => {
    ...
    describe("after deploying all staking reward contracts", async () => {
      ...
      it("gas", async () => {
        ...
        expect(receipt.gasUsed).to.eq("417069");
      });
